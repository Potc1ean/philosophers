...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/09/28 16:20:06 by ichpakov          #+#    #+#             */
/*   Updated: 2024/09/29 17:03:32 by ichpakov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_table	*init_table(int ac, char **av, int i)
{
	...
	if (!table)
		return (error_null(STR_ERR_MALLOC, NULL, 0));
	table->nb_philos = ph_atoi(av[i++]);
	table->time_to_die = ph_atoi(av[i++]);
	table->time_to_eat = ph_atoi(av[i++]);
	table->time_to_sleep = ph_atoi(av[i++]);
	table->must_eat_count = -1;
	if (ac == 6)
		table->must_eat_count = ph_atoi(av[i]);
